...
jetson_car_hotspot_connection()
{
  ...
  then
    echo "未找到热点配置文件：${JETSON_CAR_HOTSPOT_CONFIG}" >&2
    return 1
  fi
  ...
  connection="$(
    awk '
      /^[[:space:]]*hotspot_connection[[:space:]]*:/ {
        line = $0
        sub(/^[[:space:]]*hotspot_connection[[:space:]]*:[[:space:]]*/, "", line)
        sub(/[[:space:]]+#.*$/, "", line)
        gsub(/^[[:space:]]+/, "", line)
        gsub(/[[:space:]]+$/, "", line)
        print line
  ...
  then
    ...
    echo "配置文件：${JETSON_CAR_HOTSPOT_CONFIG}" >&2
    return 1
  fi

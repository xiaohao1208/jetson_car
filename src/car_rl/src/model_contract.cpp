...
// 返回当前用户的 car_rl 引擎缓存根目录
std::filesystem::path user_cache_root()
{
  if (const char * xdg_cache = std::getenv("XDG_CACHE_HOME")) {
    if (*xdg_cache != '\0') {
      return std::filesystem::path(xdg_cache) / "car_rl";
    }
  }
  if (const char * home = std::getenv("HOME")) {
    if (*home != '\0') {
      return std::filesystem::path(home) / ".cache" / "car_rl";
    }
  }
  return std::filesystem::temp_directory_path() / "car_rl";
}
...
ModelBundle load_model_bundle(const std::filesystem::path & bundle_root)
{
  ...
  require_nonempty_file(
    absolute_root, absolute_root / "controller/evaluation.json",
    "控制器评估文件");
  require_nonempty_file(
...
std::filesystem::path default_bundle_path()
{
  return std::filesystem::path(
    ament_index_cpp::get_package_share_directory("car_rl")) / "models" / "active";
}
  ...
  const std::string & precision)
{
  ...
  return user_cache_root() / short_hash /
  ...
  const std::string & runtime_fingerprint)
{
  ...
  if (runtime_fingerprint.empty()) {
    throw std::runtime_error("推理环境指纹不能为空");
  }

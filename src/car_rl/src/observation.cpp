...
GoalAlignmentResult compute_goal_alignment(
  const geometry_msgs::msg::Pose & robot_pose,
  const geometry_msgs::msg::Pose & goal_pose,
  bool position_latched,
  double xy_tolerance,
  ...
  double max_angular_speed)
{
  if (!std::isfinite(xy_tolerance) || xy_tolerance <= 0.0 ||
    ...
    !std::isfinite(angular_gain) || angular_gain <= 0.0 ||
    !std::isfinite(max_angular_speed) || max_angular_speed < 0.0)
  {
    throw std::invalid_argument("终点对齐参数无效");
  }

  GoalAlignmentResult result;
  const double position_error = distance(
    robot_pose.position.x, robot_pose.position.y,
    goal_pose.position.x, goal_pose.position.y);
  result.position_reached = position_latched || position_error <= xy_tolerance;
  if (!result.position_reached) {
    return result;
  }

  result.yaw_error = std::atan2(
    std::sin(quaternion_yaw(goal_pose.orientation) - quaternion_yaw(robot_pose.orientation)),
    std::cos(quaternion_yaw(goal_pose.orientation) - quaternion_yaw(robot_pose.orientation)));
  result.yaw_reached = std::abs(result.yaw_error) <= yaw_tolerance;
  if (!result.yaw_reached && max_angular_speed > 0.0) {
    result.command.angular.z = clamp(
      angular_gain * result.yaw_error,
      -max_angular_speed,
      max_angular_speed);
  }
  return result;

...
from car_interfaces.action import RunCalibration, RunObservationCollection
...
def _calibration_snapshot(message, goal_inflight):
    """合并状态话题和 Action 握手状态，且不覆盖已经发布的终态。"""

    state = str(message.state)
    run_id = str(message.run_id)
    handshake = bool(
        goal_inflight
        and not message.active
        and state in {"", "idle", "starting"}
        and not run_id
    )
    return {
        "active": bool(message.active or handshake),
        "state": "starting" if handshake else state,
        "phase": str(message.phase),
        "completed_trials": int(message.completed_trials),
        "total_trials": int(message.total_trials),
        "progress": float(message.progress),
        "imu_bias_radps": float(message.imu_bias_radps),
        "imu_noise_radps": float(message.imu_noise_radps),
        "imu_drift_radps": float(message.imu_drift_radps),
        "mcu_frequency_hz": float(message.mcu_frequency_hz),
        "mcu_max_gap_sec": float(message.mcu_max_gap_sec),
        "command_timeout_samples": int(message.command_timeout_samples),
        "run_id": run_id,
        "message": str(message.message),
    }


def _observation_snapshot(message, goal_inflight):
    """合并观测状态话题与本进程Action握手窗口。"""
    ...
    return {
        ...
        "state": "starting" if handshake else state,
        "phase": str(message.phase),
        "collected_samples": int(message.collected_samples),
        "target_samples": int(message.target_samples),
        "unique_samples": int(message.unique_samples),
        "route_loops": int(message.route_loops),
        "current_waypoint": int(message.current_waypoint),
        "total_waypoints": int(message.total_waypoints),
        "straight_samples": int(message.straight_samples),
        "left_turn_samples": int(message.left_turn_samples),
        "right_turn_samples": int(message.right_turn_samples),
        "near_goal_samples": int(message.near_goal_samples),
        "front_near_samples": int(message.front_near_samples),
        "left_near_samples": int(message.left_near_samples),
        "right_near_samples": int(message.right_near_samples),
        "bilateral_near_samples": int(message.bilateral_near_samples),
        "progress": float(message.progress),
        "warnings": [str(item) for item in message.warnings],
        "run_id": run_id,
        "message": str(message.message),
    }
...
class CarWebBridge(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("show_calibration_button", False)
        self.declare_parameter("show_observation_collection_button", False)
        ...
        self._map = None
        self._map_version = 0
        self._map_payload_cache_version = -1
        self._map_payload_cache = None
        ...
        self._calibration_status.state = "idle"
        self._calibration_status.phase = "idle"
        self._calibration_goal_handle = None
        self._observation_status = ObservationCollectionStatus()
        self._observation_status.state = "idle"
        self._observation_status.phase = "idle"
        self._observation_goal_handle = None
        self._obstacle_scan_status = ObstacleScanStatus()
        self._obstacle_scan_status.phase = "idle"
        ...
        self._calibration_client = ActionClient(
            self, RunCalibration, "/car/run_calibration"
        )
        self._observation_client = ActionClient(
            self,
            RunObservationCollection,
            "/car/run_observation_collection",
        )
        self._observation_cancel_client = self.create_client(
            CancelGoal,
            "/car/run_observation_collection/_action/cancel_goal",
        )
    ...
    def _on_calibration_status(self, message):
        """缓存标定节点发布的全局状态，供刷新后的网页恢复锁定。"""
        with self._lock:
            self._calibration_status = message

    def _on_observation_status(self, message):
        """缓存transient-local观测任务状态，供Web重启后恢复。"""
        with self._lock:
            self._observation_status = message

    def _on_obstacle_scan_status(self, message):
        """缓存普通导航和观测导航共用的近障扫描阶段。"""
        with self._lock:
            self._obstacle_scan_status = message
            self._last_obstacle_scan = time.monotonic()
    ...
    def status(self):
        ...
        with self._lock:
            ...
            calibration_message = self._calibration_status
            calibration_inflight = self._calibration_goal_handle is not None
            calibration = _calibration_snapshot(
                calibration_message, calibration_inflight
            )
            observation_message = self._observation_status
            observation_inflight = self._observation_goal_handle is not None
            observation = _observation_snapshot(
                observation_message, observation_inflight
            )
            scan_message = self._obstacle_scan_status
            scan_fresh = (
                now - getattr(self, "_last_obstacle_scan", 0.0) <= 1.0
            )
            obstacle_scan = {
                "active": bool(scan_fresh and scan_message.active),
                "phase": str(scan_message.phase),
                "target_offset_rad": float(
                    scan_message.target_offset_rad
                ),
                "scan_range_rad": float(scan_message.scan_range_rad),
                "message": str(scan_message.message),
            }
            hold_localized_pose = (
        ...
        e_stop_source = str(status.e_stop_source) if status else ""
        e_stop_reason = str(status.e_stop_reason) if status else ""
        if not e_stop:
            e_stop_kind = "none"
        elif e_stop_source == "car_calibrate":
            e_stop_kind = "calibration_lock"
        elif e_stop_source in {
            "car_observation_collect",
            "observation_collection_status_stale",
        }:
            e_stop_kind = "observation_lock"
        else:
            e_stop_kind = "emergency"
        return {
            ...
            "e_stop": e_stop,
            "e_stop_kind": e_stop_kind,
            "e_stop_source": e_stop_source,
    ...
    def _request_emergency_stop(self, stop, reason, source):
        """使用明确来源同步调用底盘急停服务。"""
    ...
    def emergency_stop(self, stop, reason="Web操作"):
        ...
        if not stop:
            with self._lock:
                status = self._status
                calibration_active = bool(self._calibration_status.active)
                observation_active = bool(self._observation_status.active)
            current_source = str(status.e_stop_source) if status else ""
            if current_source == "car_calibrate" and not calibration_active:
                ...
                source = (
                    "car_observation_collect"
                    if current_source == "car_observation_collect"
                    else "car_web"
                )
        return self._request_emergency_stop(stop, reason, source)

    def release_observation_lock(self, reason):
        """以观测节点身份解除其程序锁，底盘仍会拒绝覆盖真实急停。"""
        with self._lock:
            if self._observation_status.active:
                raise RuntimeError("自动观测尚未结束，拒绝解除观测安全锁")
        return self._request_emergency_stop(
            False,
            reason,
            "car_observation_collect",
        )

    def calibration_available(self):
        """标定 Action Server 已经被 ROS 图发现。"""
        return self._calibration_client.server_is_ready()

    def observation_available(self):
        """自动观测Action Server是否已被ROS图发现。"""
        return self._observation_client.server_is_ready()

    def start_observation_collection(self, waypoints, confirmations):
        """发送当前网页航点不可变副本，并同步等待Action接收。"""
        if not self._observation_client.wait_for_server(timeout_sec=1.0):
            raise RuntimeError("自动观测节点尚未就绪")
        goal = RunObservationCollection.Goal()
        goal.waypoints = [self._pose_message(item) for item in waypoints]
        goal.route_area_clear = bool(confirmations["route_area_clear"])
        goal.physical_estop_ready = bool(confirmations["physical_estop_ready"])
        goal.supervised = bool(confirmations["supervised"])
        goal.accepts_maximum_duration = bool(
            confirmations["accepts_maximum_duration"]
        )
        future = self._observation_client.send_goal_async(goal)
        deadline = time.monotonic() + 2.0
        while not future.done() and time.monotonic() < deadline:
            time.sleep(0.02)
        if not future.done() or future.result() is None:
            raise RuntimeError("自动观测请求超时")
        handle = future.result()
        if not handle.accepted:
            raise RuntimeError("自动观测请求被拒绝，请检查地图、航点和确认项")
        with self._lock:
            self._observation_goal_handle = handle
        handle.get_result_async().add_done_callback(self._observation_result)
        return True

    def cancel_observation_collection(self):
        """取消本进程或Web重启前已经存在的唯一观测Action Goal。"""
        with self._lock:
            handle = self._observation_goal_handle
        if handle is not None:
            future = handle.cancel_goal_async()
        else:
            if not self._observation_cancel_client.wait_for_service(
                timeout_sec=1.0
            ):
                raise RuntimeError("自动观测取消服务尚未就绪")
            future = self._observation_cancel_client.call_async(
                CancelGoal.Request()
            )
        deadline = time.monotonic() + 2.0
        while not future.done() and time.monotonic() < deadline:
            time.sleep(0.02)
        if not future.done() or future.result() is None:
            raise RuntimeError("取消自动观测请求超时")
        return True

    def _observation_result(self, future):
        """记录观测Action终态，详细内容继续以全局状态话题为准。"""
        try:
            wrapped = future.result()
            message = (
                wrapped.result.message
                if wrapped and wrapped.result else "结果为空"
            )
            self.get_logger().info(f"自动观测结束：{message}")
        except Exception as error:
            self.get_logger().error(f"读取自动观测结果失败：{error}")
        finally:
            with self._lock:
                self._observation_goal_handle = None

    def start_calibration(self, confirmations):
        """发送一次完整 A0～A3 标定目标并等待服务端确认接收。"""
        if not self._calibration_client.wait_for_server(timeout_sec=1.0):
            raise RuntimeError("自动标定节点尚未就绪")
        goal = RunCalibration.Goal()
        goal.straight_path_clear = bool(confirmations["straight_path_clear"])
        goal.rotation_area_clear = bool(confirmations["rotation_area_clear"])
        goal.physical_estop_ready = bool(confirmations["physical_estop_ready"])
        goal.supervised = bool(confirmations["supervised"])
        future = self._calibration_client.send_goal_async(goal)
        deadline = time.monotonic() + 2.0
        while not future.done() and time.monotonic() < deadline:
            time.sleep(0.02)
        if not future.done() or future.result() is None:
            raise RuntimeError("自动标定请求超时")
        handle = future.result()
        if not handle.accepted:
            raise RuntimeError("自动标定请求被拒绝，请检查确认项或运行状态")
        with self._lock:
            self._calibration_goal_handle = handle
        handle.get_result_async().add_done_callback(self._calibration_result)
        return True

    def _calibration_result(self, future):
        """记录长任务终态；详细状态仍以全局状态话题为准。"""
        try:
            wrapped = future.result()
            message = (
                wrapped.result.message
            ...
            self.get_logger().info(f"自动标定结束：{message}")
        except Exception as error:
            self.get_logger().error(f"读取自动标定结果失败：{error}")
        finally:
            with self._lock:
                self._calibration_goal_handle = None
    ...
    def localization_ready(self):
        """判断本次导航已收到 AMCL 位姿且当前 map TF 仍然有效"""
        with self._lock:
            amcl_confirmed = (
                self._amcl_pose_received
                and self._robot_pose is not None
                and self._robot_pose.get("source") == "amcl"
            )
        # AMCL 只在滤波器更新时发布 /amcl_pose；车辆静止时可能长时间没有
        # 新消息，但持续有效的 map -> base_footprint TF 仍能证明定位可用。
        return amcl_confirmed and self.map_transform_ready()
    ...
    def map_payload(self):
        ...
        with self._lock:
            ...
            cached = self._map_payload_cache
            cached_version = self._map_payload_cache_version
        robot_pose = self.robot_pose()
        if message is None:
            return {"available": False, "robot_pose": robot_pose}
        if cached is not None and cached_version == map_version:
            return {**cached, "robot_pose": robot_pose}
        ...
        with self._lock:
            if self._map_version == map_version:
                self._map_payload_cache_version = map_version
                self._map_payload_cache = dict(payload)
        return {**payload, "robot_pose": robot_pose}
